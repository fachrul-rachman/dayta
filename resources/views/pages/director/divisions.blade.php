<div class="flex flex-1 flex-col gap-4 rounded-xl">
    <div class="rounded-xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
        <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            ...
            <div class="grid grid-cols-2 gap-2 md:grid-cols-4 md:items-end">
                <div>
        ...
        @if ($selectedDivision)
            <div class="mt-3 grid grid-cols-2 gap-3 md:grid-cols-4">
                <x-dashboard.card
                    ...
                    :variant="match(true) {
                        $submissionHealth === null => 'default',
                        $submissionHealth >= 80 => 'success',
                        $submissionHealth >= 50 => 'warning',
                        default => 'danger',
                    }"
                ></x-dashboard.card>
                <x-dashboard.card
                ...
                <x-dashboard.card
                    ...
                    :variant="match(true) {
                        $bigRockAlignment === null => 'default',
                        $bigRockAlignment >= 60 => 'success',
                        $bigRockAlignment >= 30 => 'warning',
                        default => 'danger',
                    }"
                ></x-dashboard.card>
                <x-dashboard.card
                        ...
                        @foreach ($divisionFlags as $flag)
                            ...
                            <div class="rounded-lg border border-zinc-200 bg-zinc-50 p-3 dark:border-zinc-700 dark:bg-zinc-800">
                                <div class="flex items-center justify-between gap-2">
                                    ...
                                    <span class="inline-flex items-center rounded-md px-2 py-0.5 text-xs font-medium {{ $severityClass }}">
                                        {{ ucfirst($severity ?? 'unknown') }}
                                    </span>
                                </div>
                                <p class="mt-1 text-sm text-zinc-600 dark:text-zinc-200">
                                    {{ $flag->details }}
                                </p>
                                <p class="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
            ...
            <div class="rounded-xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-700 dark:bg-zinc-900">
                ...
                @if ($summaryHistory->isNotEmpty())
                    <div class="mt-4 border-t border-zinc-200 pt-3 dark:border-zinc-700">
                        <h4 class="text-xs font-semibold text-zinc-500 dark:text-zinc-400">
                            {{ __('Recent summaries') }}
                        </h4>
                        <div class="mt-2 flex max-h-44 flex-col gap-2 overflow-y-auto">
                            @foreach ($summaryHistory as $item)
                                <details class="group rounded-lg bg-zinc-50 p-2 dark:bg-zinc-800">
                                    <summary class="flex cursor-pointer list-none items-center justify-between gap-2">
                                        <div>
                                            ...
                                            <p class="mt-1 text-xs text-zinc-700 line-clamp-2 dark:text-zinc-200">
                                                {{ \Illuminate\Support\Str::limit($item->summary, 160) }}
                                            </p>
                                        </div>
                                        <span class="text-xs text-zinc-400 group-open:rotate-90 transition-transform">
                                            ›
                                        </span>
                                    </summary>
                                    <div class="mt-2 text-sm text-zinc-700 whitespace-pre-line dark:text-zinc-200">
                                        {{ $item->summary }}
                                    </div>
                                </details>
                            @endforeach
    ...
    @else
        <div class="rounded-xl border border-dashed border-zinc-300 bg-white p-4 text-sm text-zinc-500 shadow-sm dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-400">
